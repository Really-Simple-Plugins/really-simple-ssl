...
class rsssl_admin {
				...
				<div class="rsssl-container">
					...
					<div style="margin-left:30px">
						<?php if ( get_option( 'rsssl_before_review_notice_user' ) ) { ?>
							<p><?php printf( __( 'Hi, Really Simple SSL has kept your site secure for some time now, awesome! If you have a moment, please consider leaving a review on WordPress.org to spread the word. We greatly appreciate it! If you have any questions or feedback, leave us a %1$smessage%2$s.', 'really-simple-ssl' ), '<a href="https://really-simple-ssl.com/contact" target="_blank">', '</a>' ); ?></p>
						<?php } else { ?>
							<p><?php printf( __( 'Hi, Really Simple SSL has kept your site secure for a month now, awesome! If you have a moment, please consider leaving a review on WordPress.org to spread the word. We greatly appreciate it! If you have any questions or feedback, leave us a %1$smessage%2$s.', 'really-simple-ssl' ), '<a href="https://really-simple-ssl.com/contact" target="_blank">', '</a>' ); ?></p>
						<?php } ?>
	...
	public function get_notices_list( $args = array() ) {
		...
		foreach ( $notices as $key => $notice ) {
			...
			if ( $notice['output']['icon'] === 'warning' ) {
				$warnings[ $key ] = $notice;
			} elseif ( $notice['output']['icon'] === 'open' ) {
				$open[ $key ] = $notice;
			} else {
				$other[ $key ] = $notice;
			}
	...
	private function validate_function( $func, $is_condition = false ) {
		...
		} elseif ( strpos( $func, 'option_' ) !== false ) {
	...
	public function deactivate_popup() {
		...
		<div id="deactivate_keep_ssl" style="display: none;">
				...
				$deactivate_keep_ssl_link = add_query_arg(
					[
						'page'   => 'really-simple-security',
						'action' => 'uninstall_keep_ssl',
						'token'  => $token,
					],
					rsssl_admin_url()
				);
