<?php
/**
 * This file contains the upgrade procedure
 *
 * @since  2.4.0
...
function rsssl_upgrade() {
	...
	if ( $prev_version && version_compare( $prev_version, '5.3.0', '<=' ) ) {
		if ( file_exists( RSSSL()->admin->htaccess_file() ) && is_writable( RSSSL()->admin->htaccess_file() ) ) { //phpcs:ignore
			$htaccess      = file_get_contents( RSSSL()->admin->htaccess_file() ); //phpcs:ignore
			...
			if ( preg_match_all( $pattern_start, $htaccess ) ) {
				...
				file_put_contents( RSSSL()->admin->htaccess_file(), $htaccess ); //phpcs:ignore
			}
		}
	}
	...
	if ( $prev_version && version_compare( $prev_version, '6.0.5', '<' ) ) {
		if ( file_exists( trailingslashit( WPMU_PLUGIN_DIR ) . 'rsssl_rest_api_optimizer.php' ) ) {
			wp_delete_file( trailingslashit( WPMU_PLUGIN_DIR ) . 'rsssl_rest_api_optimizer.php' );
		}
	}
