...
const SettingsGroup = props => {
  ...
  (0,_wordpress_element__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    Promise.all(/*! import() */[__webpack_require__.e("vendors-node_modules_material-ui_core_esm_TextField_TextField_js-node_modules_react-data-tabl-841a27"), __webpack_require__.e("src_Settings_Field_js")]).then(__webpack_require__.bind(__webpack_require__, /*! ./Field */ "./src/Settings/Field.js")).then(_ref => {
      let {
        default: Field
      } = _ref;
      setField(() => Field);
  ...
  */
  ...
  // // Start by checking if we have actual groups.
  //     for (const item of subMenu.menu_items) {
  //         console.log('Checking outer item loop 1:', item.id);
  //         if (item.hasOwnProperty('groups')) {
  //             console.log('Item', item.id, 'has groups property with length:', item.groups.length);
  //         }
  //
  //         if (item.id === selectedSubMenuItem && item.hasOwnProperty('groups')) {
  //             console.log('Entered group checking loop for item:', item.id, 'with groups:', item.groups);
  //             console.log('Found item with selectedSubMenuItem and groups:', item.id, 'Groups:', item.groups);
  //             for (const group of item.groups) {
  //                 console.log('Checking group within item:', group.group_id, 'against props.group:', props.group);
  //                 if (group.group_id === props.group) {
  //                     console.log('Matched group with props.group:', props.group);
  //                     activeGroup = group;
  //                 }
  //             }
  //         }
  //     }
  //
  //     console.log("After first loop, activeGroup is:", activeGroup);
  //
  //
  // // If activeGroup is still not set, then default to the parent menu item.
  //     if (!activeGroup || !activeGroup.title) {
  //         for (const item of subMenu.menu_items) {
  //             console.log('Checking outer item loop 2:', item.id);
  //             if (item.id === selectedSubMenuItem) {
  //                 console.log('Matched outer item with selectedSubMenuItem:', selectedSubMenuItem);
  //                 activeGroup = item;
  //                 break;
  ...
  //
  //     if (activeGroup && activeGroup.groups) {
  //         let nestedGroup = activeGroup.groups.filter(group => group.group_id === props.group);
  //         if (nestedGroup.length > 0) {
  //             activeGroup = nestedGroup[0];
  //         }
  //     }
...
//# sourceMappingURL=src_Settings_Settings_js.7f176e14bdf134e89add.js.map