...
const CountryDatatable = (props) => {
	...
	createTheme(
        ...
        'light'
					...
					const blockRegionByCode = useCallback(
						(code) => {
                        if (Array.isArray( code )) {
					...
					const allowCountryByCode = useCallback(
						(code) => {
                        if (Array.isArray( code )) {
					...
					const blockCountryByCode = useCallback(
						(code) => {
                        if (Array.isArray( code )) {
                            const ids = code.map( item => item.iso2_code );
					...
					const generateGoodBad = useCallback(
						(value) => (
						value > 0 ? (
