...
function rsssl_le_read_more( $url, $add_character = ' ' ) {
	$html = sprintf(
		__(
			'For more information, please read this %1$sarticle%2$s',
			'really-simple-ssl'
		),
...
if ( ! function_exists( 'rsssl_get_manual_instructions_text' ) ) {
	...
	function rsssl_get_manual_instructions_text( $url ) {
		...
		if ( $url === $default_url ) {
			...
			$complete_manually = sprintf( __( 'Please complete %1$smanually%2$s', 'really-simple-ssl' ), '<a target="_blank" href="' . $url . '">', '</a>' );
			$activate_manually = sprintf( __( 'Please activate it on your dashboard %1$smanually%2$s', 'really-simple-ssl' ), '<a target="_blank" href="' . $url . '">', '</a>' );
		...
		if ( $activated_by_default ) {
			$msg
				= sprintf(
					__(
						'According to our information, your hosting provider supplies your account with an SSL certificate by default. Please contact your %1$shosting support%2$s if this is not the case.',
						'really-simple-ssl'
					),
					'<a target="_blank" href="' . $url . '">',
					'</a>'
		...
		} elseif ( $dashboard_activation_required ) {
			...
			$msg
				= sprintf(
					__(
						'According to our information, your hosting provider does not allow any kind of SSL installation, other then their own paid certificate. For an alternative hosting provider with SSL, see this %1$sarticle%2$s.',
						'really-simple-ssl'
					),
					'<a target="_blank" href="https://really-simple-ssl.com/hosting-providers-with-free-ssl">',
					'</a>'
				);
		} else {
			$msg = __( 'Your hosting environment does not allow automatic SSL installation.', 'really-simple-ssl' ) . ' ' .
					$complete_manually . ' ' .
					sprintf( __( 'You can follow these %1$sinstructions%2$s.', 'really-simple-ssl' ), '<a target="_blank" href="' . $default_url . '">', '</a>' ) . '&nbsp' .
					__( 'After completing the installation, you can continue to the next step to complete your configuration.', 'really-simple-ssl' )
					. $button_complete;
		}
