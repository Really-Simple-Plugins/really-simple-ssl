...
class rsssl_letsencrypt_handler {
	...
	public function check_domain() {
		...
		} elseif ( is_multisite() && get_current_blog_id() !== get_main_site_id() ) {
			rsssl_progress_remove( 'system-status' );
			$action  = 'stop';
			$status  = 'error';
			...
			try {
				...
				if ( strpos( $response, 'invalid contact domain' ) ) {
					...
					$response = __( 'The used domain for your email address is not allowed.', 'really-simple-ssl' ) . '&nbsp;' .
								sprintf( __( 'Please change your email address %1$shere%2$s and try again.', 'really-simple-ssl' ), '<a href="' . rsssl_letsencrypt_wizard_url() . '&step=2' . '">', '</a>' );
				}
					...
					if ( $order ) {
						try {
							...
							if ( strpos( $error, 'No challenge found with given type' ) !== false ) {
								...
								$order->clear();
							} elseif ( strpos( $error, 'Order has status "invalid"' ) !== false ) {
								$order->clear();
				...
				if ( isset( $response[0]['txt'] ) ) {
					if ( $response[0]['txt'] === $token ) {
						...
						$response = new RSSSL_RESPONSE(
							'error',
							$action,
							sprintf( __( 'The DNS response for %1$s was %2$s, while it should be %3$s.', 'really-simple-ssl' ), "_acme-challenge.$identifier", $response[0]['txt'], $token )
						);
						break;
					}
				} else {
	...
	public function check_host() {
		...
		if ( isset( RSSSL_LE()->hosts->hosts[ $host ] ) ) {
			...
			if ( RSSSL_LE()->hosts->hosts[ $host ]['free_ssl_available'] === 'activated_by_default' ) {
				...
				$action  = 'continue';
				$status  = 'error';
				$message = sprintf( __( 'According to our information, your hosting provider supplies your account with an SSL certificate by default. Please contact your %1$shosting support%2$s if this is not the case.', 'really-simple-ssl' ), '<a target="_blank" href="' . $url . '">', '</a>' ) . '&nbsp' .
		...
		if ( rsssl_is_ready_for( 'installation' ) ) {
			try {
				if ( $server === 'cpanel' ) {
					...
					} elseif ( function_exists( 'rsssl_shell_installSSL' ) ) {
