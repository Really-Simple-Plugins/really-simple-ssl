 ...
 */
if ( ! class_exists( 'Vulnerabilities' ) ) {
	/**
	 * Class Vulnerabilities
	 */
	class Vulnerabilities {
		public const RSS_VULNERABILITIES_LOCATION = '/really-simple-ssl';
		private const RSS_SECURITY_API            = 'https://api.really-simple-security.com/storage/downloads/';
		public $workable_plugins                  = array();
		 ...
		 */
		public static function first_run(): array {
			if ( ! rsssl_user_can_manage() ) {
				return array();
			}
			$instance = self::instance();
			...
			if ( ! wp_next_scheduled( 'Vulnerabilities_cron' ) ) {
				wp_schedule_event( time(), $instance->schedule, 'Vulnerabilities_cron' );
				//now we add the action to the cron.
				add_action( 'Vulnerabilities_cron', array( $instance, 'run' ) );
			}
		...
		public function show_help_notices( $notices ) {
			...
			foreach ( $this->risk_levels as $key => $value ) {
				...
				$notice                          = array(
					'callback'          => '_true_',
					'score'             => 1,
					'show_with_options' => array( 'enable_vulnerability_scanner' ),
					'output'            => array(
						'true' => array(
			...
			// Now we add the test notices for admin and dashboard.
			...
			if ( get_option( 'test_vulnerability_tester' ) ) {
				...
				$site_wide_icon = 'l' === $side_wide || 'm' === $side_wide ? 'open' : 'warning';
				$dashboard_icon = 'l' === $dashboard || 'm' === $dashboard ? 'open' : 'warning';
				if ( 'l' === $dashboard || 'm' === $dashboard || 'h' === $dashboard || 'c' === $dashboard ) {
					$notices[ 'test_vulnerability_sitewide_' . get_option( 'test_vulnerability_tester' ) ] = array(
						'callback'          => '_true_',
						'score'             => 1,
						'show_with_options' => array( 'enable_vulnerability_scanner' ),
						'output'            => array(
				...
				if ( $dashboard_icon !== $site_wide_icon ) {
					if ( 'l' === $side_wide || 'm' === $side_wide || 'h' === $side_wide || 'c' === $side_wide ) {
						$notices[ 'test_vulnerability_dashboard_' . get_option( 'test_vulnerability_tester' ) ] = array(
							'callback'          => '_true_',
							'score'             => 1,
							'show_with_options' => array( 'enable_vulnerability_scanner' ),
							'output'            => array(
		...
		public static function testGenerator(): array {
			...
			if ( 'l' === $mail_notification || 'm' === $mail_notification || 'h' === $mail_notification || 'c' === $mail_notification ) {
		...
		public static function get_stats( $stats ): array {
			...
			$risk_data                = $self->measures_data();
			$stats['riskData']        = $risk_data['data'];
		...
		public function cache_installed_plugins(): void {
				...
				if ( ! empty( $components ) ) {
					foreach ( $components as $component ) {
						if ( $plugin['TextDomain'] === $component->slug ) {
							if ( ! empty( $component->vulnerabilities ) && true === $plugin['folder_exists'] ) {
								...
								$plugin['date']           = $this->get_linked_date( $component->vulnerabilities, $plugin['risk_level'] );
								$plugin['file']           = $key;
							}
						}
					}
			...
			if ( isset( $update->updates[0]->response ) && 'upgrade' === $update->updates[0]->response ) {
			...
			if ( ! empty( $core->vulnerabilities ) ) {
				...
				$core_plugin['date']           = $this->get_linked_date( $core->vulnerabilities, $core_plugin['risk_level'] );
		...
		public static function add_startup_notices( $notices ) {
			...
			$notices['Vulnerabilities'] = array(
				'callback' => 'Vulnerabilities_enabled',
				'score'    => 3,
				'output'   => array(
					'true'  => array(
		...
		public static function measures_data(): array {
			...
			$measures[] = array(
				...
				'description' => sprintf( __( 'Forcefully update the plugin or theme with this option.', 'really-simple-ssl' ), self::risk_naming( 'l' ) ),
			);
			$measures[] = array(
				'id'          => 'quarantine',
				'name'        => __( 'Quarantine', 'really-simple-ssl' ),
				'value'       => get_option( 'rsssl_quarantine' ),
				'description' => sprintf( __( 'Isolates the plugin or theme if no update can be performed', 'really-simple-ssl' ), self::risk_naming( 'm' ) ),
			);
		...
		public static function risk_vulnerabilities_data( array $response, string $action, $data ): array {
			...
			if ( 'risk_vulnerabilities_data_save' === $action ) {
		...
		public function add_vulnerability_field( $column_name, $plugin_file ) {
			if ( 'vulnerability' === $column_name ) {
				if ( $this->check_vulnerability( $plugin_file ) ) {
					$identifier = $this->getIdentifier( $plugin_file );
					$severity   = $this->check_severity( $plugin_file );
					$named_risk = ucfirst( $this->risk_naming( $severity ) );
					$url        = esc_url( 'https://really-simple-ssl.com/vulnerabilities/' . $identifier );

					switch ( $severity ) {
						case 'c':
							echo sprintf( '<a class="btn-vulnerable critical" target="_blank" href="%s">%s</a>', $url, $named_risk );

							break;

						case 'h':
							echo sprintf( '<a class="btn-vulnerable high" target="_blank" href="%s">%s</a>', $url, $named_risk );

							break;

						case 'm':
							echo sprintf( '<a class="btn-vulnerable medium-risk" target="_blank" href="%s">%s</a>', $url, $named_risk );

							break;

						default:
							echo sprintf( '<a class="btn-vulnerable low" target="_blank" href="%s">%s</a>', $url, $named_risk );
		...
		public function get_file_stored_info( $is_core = false, $manifest = false ) {
			if ( ! rsssl_user_can_manage() ) {
				return false;
			}
			...
			$file = $upload_dir . '/' . ( $is_core ? 'core.json' : 'components.json' );
			if ( ! file_exists( $file ) ) {
				return false;
			}
		...
		public function show_theme_warning( $hook ) {
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_theme_warning_scripts' ) );
		}
		...
		public function enqueue_theme_warning_scripts( $hook ) {
			...
			wp_add_inline_script( 'Vulnerabilities', $inline_script );
		}

		public function show_theme_closed() {
			$screen = get_current_screen();
			$theme  = wp_get_theme();
			if ( $screen && 'settings_page_really-simple-security' !== $screen->id ) {
				add_action(
					'admin_notices',
					function () use ( $theme ) {
						?>
						<div class="notice notice-error is-dismissible">
							<p>
								...
								echo sprintf(
									__(
										esc_attr(
											'The theme %s is closed for security issues. Please update the theme as soon as possible.'
										),
										'rss-security'
									),
									$theme->get( 'Name' )
			...
			foreach ( $components as $component ) {
				foreach ( $active_plugins as $active_plugin ) {
					...
					if ( isset( $component->slug ) && $component->slug === $active_plugin['TextDomain'] ) {
						...
						if ( count( $component->vulnerabilities ) > 0 || 'closed' === $component->status ) {
		...
		private function get_highest_vulnerability( $vulnerabilities ): string {
			...
			foreach ( $vulnerabilities as $vulnerability ) {
				if ( null === $vulnerability->severity ) {
					continue;
				}
		...
		private function enable_feedback_in_plugin() {
			...
			add_action( 'admin_enqueue_scripts', [ $this, 'add_vulnerability_styles' ] );
			//we add an extra column to the plugins page
			add_filter( 'manage_plugins_columns', [ $this, 'add_vulnerability_column' ] );
			//now we add the field to the plugins page
			add_action( 'manage_plugins_custom_column', [ $this, 'add_vulnerability_field' ], 10, 3 );
		}
		...
		private function filter_vulnerabilities( $vulnerabilities, $Version, $core = false ): array {
			...
			foreach ( $vulnerabilities as $vulnerability ) {
				...
				if ( 'not fixed' !== $vulnerability->fixed_in ) {
		...
		private function get_linked_date( $vulnerabilities, string $risk_level ) {
			foreach ( $vulnerabilities as $vulnerability ) {
				if ( $vulnerability->severity === $risk_level ) {
					//we return the date in a readable format
					return gmdate( get_option( 'date_format' ), strtotime( $vulnerability->published_date ) );
				}
			}
		}

		private function assemble_first_run(): array {
			$this->check_files();
			$this->cache_installed_plugins();
	...
	add_action( 'admin_init', array( Vulnerabilities::class, 'instance' ) );
}
...
add_filter( 'rsssl_notices', array( new Vulnerabilities(), 'show_help_notices' ), 10, 1 );

if ( ! function_exists( 'Vulnerabilities_enabled' ) ) {
	/**
	 * This function checks if the vulnerability scanner is enabled is being used as callback for the notices.
	 */
	function Vulnerabilities_enabled(): bool {
		return rsssl_get_option( 'enable_vulnerability_scanner' );
	}
}
function Vulnerabilities_api( array $response, string $action, $data ): array {
	if ( ! rsssl_user_can_manage() ) {
		return $response;
	}
	...
	switch ( $action ) {
		case 'vulnerabilities_test_notification':
			...
			$response = Vulnerabilities::testGenerator();

			break;

		case 'vulnerabilities_scan_files':
			$response = Vulnerabilities::first_run();

			break;

		case 'vulnerabilities_measures_get':
			$response = Vulnerabilities::measures_data();
...
add_filter( 'rsssl_do_action', 'Vulnerabilities_api', 10, 3 );
...
if ( ! function_exists( 'rsssl_store_measures' ) ) {
	...
	function rsssl_store_measures( $data ): array {
		...
		return Vulnerabilities::measures_data();
	}
}
