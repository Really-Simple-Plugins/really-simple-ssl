...
if ( ! class_exists( 'rsssl_vulnerabilities' ) ) {
	...
	class rsssl_vulnerabilities {
		...
		public function cache_installed_plugins( $force_update = false ): void {
			...
			$installed_themes = array_map(
				static function ( $theme ) use ( $update ) {
					return [
						'Name'             => $theme->get( 'Name' ),
			...
			$installed_plugins = array_map(
				static function ( $plugin, $slug ) use ( $update ) {
					$plugin['type']             = 'plugin';
			...
			if ( ( $column_name === 'vulnerability' ) ) {
				...
				if ( $this->check_vulnerability( $plugin_file ) ) {
					switch ( $this->check_severity( $plugin_file ) ) {
						case 'c':
							echo sprintf(
								'<a class="rsssl-btn-vulnerable rsssl-critical" target="_blank" href="%s">%s</a>',
								'https://really-simple-ssl.com/vulnerabilities/' . $this->getIdentifier( $plugin_file ),
								ucfirst( $this->risk_naming['c'] )
							);
							break;
						case 'h':
							echo sprintf(
								'<a class="rsssl-btn-vulnerable rsssl-high" target="_blank" href="%s">%s</a>',
								'https://really-simple-ssl.com/vulnerabilities/' . $this->getIdentifier( $plugin_file ),
						...
						case 'm':
							echo sprintf(
								'<a class="rsssl-btn-vulnerable rsssl-medium" target="_blank" href="%s">%s</a>',
								'https://really-simple-ssl.com/vulnerabilities/' . $this->getIdentifier( $plugin_file ),
						...
						default:
							echo sprintf(
								'<a class="rsssl-btn-vulnerable rsssl-low" target="_blank" href="%s">%s</a>',
								'https://really-simple-ssl.com/vulnerabilities/' . $this->getIdentifier( $plugin_file ),
				...
				if ( $this->is_quarantined( $plugin_file ) ) {
					echo sprintf(
						'<a class="rsssl-btn-vulnerable rsssl-critical" target="_blank" href="%s">%s</a>',
						'https://really-simple-ssl.com/instructions/about-vulnerabilities/#quarantine',
						__( 'Quarantined', 'really-simple-ssl' )
					);
				}
			}
		}
		...
		public function send_vulnerability_mail(): void {
			...
			$mailer->title          = sprintf( __( '%1$s: %2$s vulnerabilities found', 'really-simple-ssl' ), $this->date(), $total );
		...
		protected function createBlock( string $risk_level, int $count ): array {
			...
			$title   = $this->get_warning_string( $risk_level, $count );
			$message = $count === 1 ? sprintf( __( 'A %1$s vulnerability is found in %2$s.', 'really-simple-ssl' ), $risk, $plugin_name ) : sprintf( __( 'Multiple %s vulnerabilities have been found.', 'really-simple-ssl' ), $risk );
