...
//        add_filter( 'manage_users_custom_column', array( __CLASS__, 'manage_users_custom_column' ), 10, 3 );
    ...
    public static function load_textdomain() {
        load_plugin_textdomain( 'really-simple-ssl' );
    }
...
//            'Two_Factor_Dummy'        => TWO_FACTOR_DIR . 'providers/class-two-factor-dummy.php',
        ...
        if ( isset( $providers['Two_Factor_FIDO_U2F'] ) && version_compare( PHP_VERSION, '5.3.0', '<' ) ) {
            ...
            trigger_error( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_trigger_error
                ...
                /* translators: %s: version number */
                    __( 'FIDO U2F is not available because you are using PHP %s. (Requires 5.3 or greater)', 'really-simple-ssl' ), // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                    PHP_VERSION
                )
            );
        }
    ...
    public static function filter_authenticate( $user ) {
        if ( $user instanceof WP_User && self::is_api_request() && self::is_user_using_two_factor( $user->ID ) && ! self::is_user_api_login_enabled( $user->ID ) ) {
            return new WP_Error(
                ...
                __( 'Error: API login for user disabled.', 'really-simple-ssl' )
            );
        }
    ...
    public static function login_form_validate_2fa() {
        ...
        if ( true === self::is_user_rate_limited( $user ) ) {
            ...
            $error = new WP_Error(
                'two_factor_too_fast',
                sprintf(
                    __( 'ERROR: Too many invalid verification codes, you can try again in %s. This limit protects your account against automated attacks.', 'really-simple-ssl' ),
        ...
        if ( true !== $provider->validate_authentication( $user ) ) {
            do_action( 'wp_login_failed', $user->user_login, new WP_Error( 'two_factor_invalid', __( 'ERROR: Invalid verification code.', 'really-simple-ssl' ) ) );
            ...
            self::login_html( $user, $login_nonce['key'], $_REQUEST['redirect_to'], esc_html__( 'ERROR: Invalid verification code.', 'really-simple-ssl' ), $provider );
            exit;
        }
    ...
    public static function filter_manage_users_columns( array $columns ) {
        $columns['really-simple-ssl'] = __( 'really-simple-ssl', 'really-simple-ssl' );
        return $columns;
    }
    ...
    public static function manage_users_custom_column( $output, $column_name, $user_id ) {
        ...
        if ( 'really-simple-ssl' !== $column_name ) {
            return $output;
        }

        if ( ! self::is_user_using_two_factor( $user_id ) ) {
            return sprintf( '<span class="dashicons-before dashicons-no-alt">%s</span>', esc_html__( 'Disabled', 'really-simple-ssl' ) );
        } else {
    ...
    public static function user_two_factor_options( $user ) {
        ...
        <table class="form-table" id="two-factor-options">
            <tr>
                <th>
                    <?php esc_html_e( 'Two-Factor Options', 'really-simple-ssl' ); ?>
                </th>
                <td>
                    <table class="two-factor-methods-table">
                        <thead>
                        <tr>
                            <th class="col-enabled" scope="col"><?php esc_html_e( 'Enabled', 'really-simple-ssl' ); ?></th>
                            <th class="col-primary" scope="col"><?php esc_html_e( 'Primary', 'really-simple-ssl' ); ?></th>
                            <th class="col-name" scope="col"><?php esc_html_e( 'Type', 'really-simple-ssl' ); ?></th>
                        </tr>
                        </thead>
                        <tbody>
